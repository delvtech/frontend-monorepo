...
export const buildBaseTokenInfo = async (principalTokenInfo: PrincipalTokenInfo): Promise<BaseTokenInfo> => {
    ...
    const addresses = zx
        ...
        .nonempty()
        .parse(
            Array.from(new Set(Object.values(principalTokenInfo.metadata).map(({ underlying }) => underlying)))
        )
